...
export function computeRecentBand(
  ...
  const avg =
    recent.reduce((sum, s) => sum + (s.overall_band ?? 0), 0) /
    recent.length;
...
export function buildPersonalisation(
  ...
  const difficulty =
    computedBand < 5.5 ? 'foundation' : computedBand < 7.0 ? 'intermediate' : 'advanced';

  const criterionAvgs = (['TR', 'CC', 'LR', 'GRA'] as const).map((key) => ({
    key,
    avg: submissions.slice(0, 3).length
      ? submissions
          .slice(0, 3)
          .reduce(
            (s, sub) => s + (sub.criteria_scores?.[key] ?? 0),
            0,
          ) / Math.min(submissions.length, 3)
      : 0,
  }));

  criterionAvgs.sort((a, b) => a.avg - b.avg);

  const emphasisCriteria = criterionAvgs
    .slice(0, 2)
    .map((c) => c.key) as PersonalisationProfile['emphasisCriteria'];

  const task2Ratio = submissions.length
    ? submissions.filter((s) => s.task_type === 'task2').length /
      submissions.length
    : 0.5;
  ...
  const weakNames = emphasisCriteria
    .map(
      (k) =>
        ({
          TR: 'Task Response',
  ...
  const coachingNote =
    bandGap <= 0
      ? 'You have reached your target band. Set a new goal.'
      : bandGap <= 0.5
      ? `You are close to your target. Focus on ${weakNames} for the final push.`
